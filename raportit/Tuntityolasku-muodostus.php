...
echo('<p>--------------------------------------------------------</p>');
echo('<form method="post" action="Tuntityolasku-muodostus.php">');
...
function esita_tarvikkeiden_tiedot($sopimus_id) {
    echo("<p>-----------</p>");
    echo("<p><b>KÄYTETTYJEN TARVIKKEIDEN TIEDOT</b> (ALV-kanta 24%)</p>");
    echo("<table border='1'>");
    echo("<tr><th>Nimi</th><th>Käytetty määrä</th><th>Yksikköhinta</th><th>Hinta ilman ALV</th><th>ALV-osuus</th><th>Lopullinen hinta</th></tr>");
    ...
    $tarvike_tiedot_kysely = "SELECT t.tarvike_id, nimi, yksikko, maara, myyntihinta FROM tarvikelista AS tl JOIN tarvike AS t ON tl.tarvike_id = t.tarvike_id WHERE tl.sopimus_id = $sopimus_id AND tl.maara > 0;";
    ...
    if($tarvike_tiedot_tulos) {
        while($rivi = pg_fetch_row($tarvike_tiedot_tulos)) {
            ...
            $hinta_ilman_alv = $tarvike_hinta * $tarvike_maara;
...
function esita_tuntityot_tiedot($sopimus_id) {
    ...
    echo("<table border='1'>");
    echo("<tr><th>Nimi</th><th>Tunnit</th><th>Tuntihinta</th><th>Hinta ilman ALV</th><th>ALV-osuus</th><th>Lopullinen hinta</th><th>Kotitalousvähennettävä</th></tr>");
    ...
    $tyotunnit_tiedot_kysely = "SELECT tt.tyotyyppi_id, tyyppinimi, tuntimaara, tuntihinta, kotitalousvahennettava FROM tyosuoritus AS ts JOIN tyotyyppi AS tt ON ts.tyotyyppi_id = tt.tyotyyppi_id WHERE ts.sopimus_id = $sopimus_id AND tuntimaara > 0;";
    ...
    if($tyotunnit_tiedot_tulos) {
        while($rivi = pg_fetch_row($tyotunnit_tiedot_tulos)) {
            ...
            $hinta_ilman_alv = $tyotyyppi_hinta * $tyotyyppi_tunnit;
            $alv_osuus = 0.24 * $hinta_ilman_alv;
            $lopullinen_hinta = $hinta_ilman_alv + $alv_osuus;
